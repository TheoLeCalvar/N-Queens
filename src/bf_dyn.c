...
size_t  bf_dyn_get_next_bit(const bf_dyn_t* bf, size_t cur) {
        ...
        if (cur % BF_DYN_CHUNK_SIZE == BF_DYN_CHUNK_SIZE - 1) {
                ++i;
                goto simple;
        }
...
int     bf_dyn_contains(const bf_dyn_t* a, const bf_dyn_t* b) {
        log_info("bf_dyn_contains not implemented yet.");
        return 0;
}
...
void    bf_dyn_to_str(const bf_dyn_t* bf, char* buf) {
        log_info("bf_dyn_to_str not implemented yet.");
}
...
int main(int argc, char** argv) {
        ...
        bf_dyn_set(a, 15);
        TEST_ASSERT("Check bf_dyn_set & bf_dyn_get", bf_dyn_get(a, 15));
        ...
        bf_dyn_init(&a, 512);
        bf_dyn_diff(&a, &b);
        TEST_ASSERT("Check bf_dyn_diff", !bf_dyn_get(a, 47));
