...
#define bf_dyn_get(__bf,n) \
        ((__bf->bf[(n) / 64] >> ((n) & 63)) & 1LL)

#define bf_dyn_set(__bs,n) ({                             \
        __bs->nb_set += bf_dyn_get(__bs, n) == 0;          \
        (__bs->bf[(n) / 64] |= (1LL << ((n) & 63)));      \
        })

#define bf_dyn_unset(__bs,n) ({                           \
        __bs->nb_set -= bf_dyn_get(__bs, n) > 0;          \
        __bs->bf[(n) / 64] &= ~(1LL << ((n) & 63));       \
        })
...
#define bf_dyn_get(__bf,n) \
        ((__bf->bf[(n) / 32] >> ((n) & 31)) & 1LL)

#define bf_dyn_set(__bs,n) ({                             \
        __bs->nb_set += bf_dyn_get(__bs, n) == 0;            \
        __bs->bf[(n) / 32] |= (1LL << ((n) & 31));        \
        })
